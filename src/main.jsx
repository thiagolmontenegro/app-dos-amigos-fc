...
function AppDosAmigosFC() {
  ...
  async function signUp() {
    ...
    if (!name || !email || !authForm.password) return setAuthMsg("Preencha nome, e-mail e senha.");
  ...
  async function linkCurrentUserByEmail() {
    ...
    const email = String(user.email || "").trim().toLowerCase();
    const player = players.find((p) => String(p.email || "").trim().toLowerCase() === email);
    if (!player) return setAuthMsg("Não encontrei cadastro na tabela players com este e-mail.");
    await supabase.from("players").upsert({ ...player, user_id: user.id, email });
    await loadPlayersFromDb();
    notifyListChange("Cadastro vinculado com sucesso.");
  }
