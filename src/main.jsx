...
function AppDosAmigosFC() {
  ...
  function SecurityScreen() {
    ...
    return <div className="space-y-6">
      <Box title="🛡️ Segurança e organização">
        <div className="grid md:grid-cols-4 gap-3">
          <StatCard label="Admins" value={admins.length} tone="bg-emerald-950/70" />
          <StatCard label="Financeiro" value={finance.length} tone="bg-blue-950/70" />
          <StatCard label="Pendentes" value={pending.length} tone={pending.length ? "bg-yellow-950/70" : "bg-zinc-800"} />
          <StatCard label="Logs" value={auditLogs.length} />
        </div>

        <div className="flex flex-col md:flex-row gap-2">
          <Button onClick={refreshSecurityData} className="bg-blue-500 text-black">Atualizar segurança</Button>
          <Button onClick={() => setScreen("players")} className="bg-emerald-500 text-black">Gerenciar jogadores</Button>
        </div>

        <div className="bg-zinc-950 border border-zinc-800 rounded-3xl p-4 space-y-3">
          <h3 className="text-lg font-black">Permissões dos usuários</h3>
          <div className="overflow-x-auto">
            <table className="w-full text-sm border-separate border-spacing-y-2">
              <thead className="text-zinc-400">
                <tr>
                  <th className="text-left px-3">Usuário</th>
                  <th className="text-left px-3">E-mail</th>
                  <th>Perfil</th>
                  <th>Aprovado</th>
                  <th>User ID</th>
                </tr>
              ...
              <tbody>
                {players.map((p) => <tr key={p.id} className="bg-zinc-900 border border-zinc-800">
                  <td className="p-3 font-black rounded-l-xl">{displayName(p)}</td>
                  <td className="p-3 text-zinc-400">{p.email || "-"}</td>
                  <td className="text-center"><span className={`${roleBadgeClass(p)} px-2 py-1 rounded-xl text-xs font-black`}>{roleLabel(p)}</span></td>
                  <td className="text-center">{p.approved ? "Sim" : "Não"}</td>
                  <td className="text-center text-xs text-zinc-500 rounded-r-xl">{p.user_id ? "vinculado" : "sem login"}</td>
                </tr>)}
              </tbody>
            </table>
          </div>
      ...
      <Box title="📜 Logs de auditoria">
        {auditLogs.length === 0 ? <p className="text-zinc-500">Nenhum log registrado ainda.</p> : <div className="space-y-2 max-h-[520px] overflow-auto pr-1">
          {auditLogs.map((log) => <div key={log.id} className="bg-zinc-800 border border-zinc-700 rounded-2xl p-3">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-1">
              <b className="text-white">{log.action}</b>
              <span className="text-xs text-zinc-500">{log.created_at ? new Date(log.created_at).toLocaleString("pt-BR") : ""}</span>
            </div>
            <p className="text-xs text-zinc-400 mt-1">Por: {log.actor_email || "-"}</p>
            <pre className="text-xs text-zinc-500 bg-zinc-950 rounded-xl p-2 mt-2 overflow-auto">{JSON.stringify(log.details || {}, null, 2)}</pre>
          </div>)}
        </div>}
      </Box>
    </div>;
    ...
    return <header className="bg-zinc-950/95 border border-zinc-800 rounded-[24px] p-3 shadow-2xl sticky top-2 z-50 backdrop-blur">
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          <h1 className="text-[21px] sm:text-3xl font-black leading-none whitespace-nowrap">App dos Amigos FC</h1>
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          {isAdmin && <button className="w-11 h-11 rounded-full border border-zinc-700 bg-zinc-950/80 flex items-center justify-center text-lg shadow-lg active:scale-95 transition" onClick={() => setHide(!hide)} title="Níveis">👁️</button>}
          {user && <button className="w-11 h-11 rounded-full border border-zinc-700 bg-zinc-950/80 flex items-center justify-center text-xl shadow-lg active:scale-95 transition" onClick={signOut} title="Sair">↪</button>}
        </div>
      </div>
      {(isAdmin || isFinanceResponsible) && <PremiumNavStable />}
    </header>;
  }

  function PremiumNavStable() {
    const nav = isAdmin
      ? [["home", "⌂", "Início"], ["players", "♚", "Jogadores"], ["match", "⚽", "Times"], ["cash", "💰", "Caixa"], ["security", "🛡️", "Segurança"]]
      : [["home", "⌂", "Início"], ["cash", "💰", "Caixa"]];

    return <nav className="mt-4 border-t border-zinc-800 pt-3 overflow-x-auto no-scrollbar">
      <div className="grid gap-1" style={{ gridTemplateColumns: `repeat(${nav.length}, minmax(96px, 1fr))` }}>
        {nav.map(([key, icon, label]) => <button key={key} onClick={() => setScreen(key)} className={`h-14 flex flex-col items-center justify-center gap-1 border-b-2 transition active:scale-[0.98] ${screen === key ? "border-yellow-400 text-yellow-400" : "border-transparent text-zinc-400"}`}>
  ...
  return <main className="min-h-screen bg-black text-white p-2 sm:p-4 md:p-8 space-y-5 md:space-y-6"><PremiumHeaderStable />
