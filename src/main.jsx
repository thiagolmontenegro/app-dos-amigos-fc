...
function AppDosAmigosFC() {
  ...
  const goalkeeperSlotsFilled = Math.min(activeGoalkeepers.length, 2);
  const lineSlotsFilled = Math.min(activeLine.length, 12);
  const remainingGoalkeeperSlots = Math.max(0, 2 - goalkeeperSlotsFilled);
  const remainingLineSlots = Math.max(0, 12 - lineSlotsFilled);
  const remainingTotalSlots = remainingGoalkeeperSlots + remainingLineSlots;
  const listIsFull = goalkeeperSlotsFilled === 2 && lineSlotsFilled === 12;
  ...
  async function savePlayer() {
    ...
    setForm({ name: "", nickname: "", level: 3, position: "Linha" });
    setEditId(null);
    await loadPlayersFromDb();
  }
  ...
  async function updatePlayerStatus(id, status) {
    ...
    if (!listActive && effectiveMode === "atleta") return notifyListChange("Nenhuma lista ativa no momento. Aguarde o Admin abrir a lista.");
  ...
  async function confirmRemovePlayer() {
    ...
    const { error } = await supabase.from("players").delete().eq("id", String(deleteTarget.id));

    if (error) return setDbMsg(error.message || "Erro ao remover jogador");
      ...
      {!listActive ? <div className="text-center py-10"><p className="text-3xl font-black">Nenhuma lista ativa</p><p className="text-zinc-400 mt-2">Aguardando o Admin abrir a lista do próximo jogo.</p></div> : <div className="space-y-5">
        <div className="bg-zinc-950 border border-emerald-800 rounded-3xl p-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
            <StatCard label="Vagas linha" value={`${lineSlotsFilled}/12`} tone={remainingLineSlots === 0 ? "bg-emerald-950/60" : "bg-zinc-800"} />
            <StatCard label="Vagas goleiro" value={`${goalkeeperSlotsFilled}/2`} tone={remainingGoalkeeperSlots === 0 ? "bg-emerald-950/60" : "bg-zinc-800"} />
            <StatCard label="Suplentes" value={reserveList.length} tone={reserveList.length ? "bg-yellow-950/60" : "bg-zinc-800"} />
            <StatCard label="Status" value={listLocked ? "Bloqueada" : listIsFull ? "Completa" : "Aberta"} tone={listLocked ? "bg-red-950/60" : listIsFull ? "bg-emerald-950/60" : "bg-blue-950/60"} />
          </div>
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2 bg-zinc-900/70 border border-zinc-800 rounded-2xl p-3 mb-4">
            <p className="text-sm text-zinc-300">
              {listIsFull ? "A lista principal está completa. Novas confirmações de jogo entram em Suplentes." : `Ainda restam ${remainingTotalSlots} vaga(s) na lista principal.`}
            </p>
          </div>
  ...
  return <main className="min-h-screen bg-zinc-950 text-white p-4 md:p-8 space-y-6"><Header />{notice && <div className="bg-emerald-500 text-black rounded-2xl p-3 font-bold shadow-xl">{notice}</div>}{screen === "home" && <div className="space-y-6">{liveEvents.length > 0 && <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-4 space-y-2"><h3 className="font-black text-white">🔔 Atualizações em tempo real</h3>{liveEvents.map((e) => <p key={e.id} className="text-sm text-zinc-300"><span className="text-emerald-400 font-bold">{e.time}</span> — {e.message}</p>)}</div>}<ListBoard />{effectiveMode === "atleta" && <Box title="🙋 Confirmar presença">
        {!listActive && (
          <div className="bg-zinc-800 border border-zinc-700 rounded-2xl p-4 text-zinc-300 font-bold">
            ⏳ Nenhuma lista ativa no momento. Aguarde o Admin abrir a lista.
          </div>
        )}

        {listActive && !listLocked && (
          <div className={listIsFull ? "bg-yellow-950/50 border border-yellow-700 rounded-2xl p-4 text-yellow-300 font-bold" : "bg-emerald-950/50 border border-emerald-700 rounded-2xl p-4 text-emerald-300 font-bold"}>
            {listIsFull
              ? "✅ Lista principal completa: próximos confirmados entram como suplentes."
              : `🟢 Lista aberta: ${remainingTotalSlots} vaga(s) restantes — ${remainingGoalkeeperSlots} goleiro(s) e ${remainingLineSlots} linha.`}
          </div>
        )}
