...
function AppDosAmigosFC() {
  ...
  <PremiumNotificationsModal />{notice && <div className="bg-emerald-500 text-black rounded-2xl p-3 font-bold shadow-xl">{notice}</div>}{gameClosed && <div className="bg-yellow-500 text-black rounded-2xl p-3 font-black shadow-xl">✅ Último jogo encerrado e salvo no histórico. Abra uma nova lista para o próximo jogo.</div>}{screen === "home" && <div className="space-y-5">{isAdmin ? <PremiumAdminDashboard /> : <PremiumAthleteDashboard />}{liveEvents.length > 0 && <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-4 space-y-2"><h3 className="font-black text-white">🔔 Atualizações em tempo real</h3>{liveEvents.map((e) => <p key={e.id} className="text-sm text-zinc-300"><span className="text-emerald-400 font-bold">{e.time}</span> — {e.message}</p>)}</div>}<ListBoard />{effectiveMode === "atleta" && <><PremiumAthleteListBox /><PremiumAthleteStats /></>}{!isAdmin && isFinanceResponsible && <div className="grid grid-cols-1 gap-3 md:gap-4">
