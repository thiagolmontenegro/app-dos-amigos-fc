...
function AppDosAmigosFC() {
  ...
  function saveRememberedLogin() {
    if (rememberLogin && form.email) {
      localStorage.setItem("afc_remember_login", "true");
      localStorage.setItem("afc_saved_email", form.email.trim().toLowerCase());
    } else {
  ...
  async function deleteCashMovement(id) {
    ...
    if (!window.confirm("Remover este lançamento? O saldo atual não será recalculado automaticamente.")) return;

    const { error } = await supabase.from("cash_movements").delete().eq("id", id);
    if (error) return notifyListChange("Erro ao remover lançamento: " + error.message);

    await loadCashMovements();
    notifyListChange("Lançamento removido do histórico.");
  }
  ...
  const premiumCard = "bg-zinc-900/80 border border-zinc-700/80 rounded-[28px] shadow-2xl backdrop-blur";
  const goldPanel = "bg-gradient-to-br from-yellow-400 via-yellow-600 to-zinc-950 border border-yellow-400/70 rounded-[30px] shadow-2xl";
  const glassCard = "bg-black/45 border border-yellow-500/30 rounded-2xl backdrop-blur shadow-xl";
  const iconBtn = "w-11 h-11 sm:w-14 sm:h-14 rounded-full bg-zinc-950/80 border border-zinc-700 flex items-center justify-center text-lg sm:text-2xl shadow-lg active:scale-95 transition";
  const iconBtnGold = "w-11 h-11 sm:w-14 sm:h-14 rounded-full bg-yellow-500 text-black border border-yellow-300 flex items-center justify-center text-lg sm:text-2xl shadow-lg shadow-yellow-500/20 active:scale-95 transition";
  const miniLabel = "text-[11px] uppercase tracking-[0.22em] font-black text-yellow-400";
  ...
  function paymentLabelFor(p) {
    ...
    if (p.position === "Goleiro") return "ISENTO";
    return pay[p.id] ? "Pago" : "Pendente";
  }

  function athleteRankingInfo() {
    if (!currentPlayer) return "-";
    const ranking = typeof rankingStats === "function" ? rankingStats() : [];
    const idx = ranking.findIndex((r) => r.name === currentPlayer.name);
    if (currentPlayer.position === "Goleiro") {
      const stats = currentPlayerMonthlyStats();
  ...
  function PremiumHeader() {
    ...
    return <header className="bg-zinc-950/95 border border-zinc-800 rounded-[24px] p-3 shadow-2xl sticky top-2 z-50 backdrop-blur">
      <div className="flex items-center gap-3">
        <img src={APP_ICON_SRC} alt="Amigos FC" className="w-16 h-16 rounded-2xl object-cover border border-yellow-400/50 shadow-lg shrink-0" />

        <div className="min-w-0 flex-1">
          <h1 className="text-[22px] sm:text-3xl font-black leading-none whitespace-nowrap">
            App dos Amigos FC
          </h1>
          <p className="text-emerald-400 font-bold text-sm mt-2 whitespace-nowrap">● Banco conectado</p>
        </div>

        <div className="flex items-center gap-2 shrink-0">
          <button className="relative w-11 h-11 rounded-full border border-zinc-700 bg-zinc-950/80 flex items-center justify-center text-xl shadow-lg active:scale-95 transition" onClick={() => setShowNotifications(true)} title="Notificações">
            🔔
          ...
          {isAdmin && <button className="w-11 h-11 rounded-full border border-zinc-700 bg-zinc-950/80 flex items-center justify-center text-lg shadow-lg active:scale-95 transition relative" onClick={() => setMode(mode === "admin" ? "atleta" : "admin")} title="Modo">
            🧑‍💼
            {effectiveMode === "admin" && <span className="absolute -bottom-2 bg-yellow-400 text-black text-[9px] font-black px-1.5 py-0.5 rounded">ADMIN</span>}
          </button>}

          {isAdmin && <button className="w-11 h-11 rounded-full border border-zinc-700 bg-zinc-950/80 flex items-center justify-center text-lg shadow-lg active:scale-95 transition" onClick={() => setHide(!hide)} title="Níveis">
            👁️
          </button>}

          {user && <button className="w-11 h-11 rounded-full border border-zinc-700 bg-zinc-950/80 flex items-center justify-center text-xl shadow-lg active:scale-95 transition" onClick={signOut} title="Sair">
            ↪
          </button>}
        </div>
      </div>

      {(isAdmin || isFinanceResponsible) && <PremiumNav />}
    </header>;
  }
  ...
  function PremiumNav() {
    const nav = isAdmin
      ...
      : [["home", "⌂", "Início"], ["cash", "💰", "Caixa"]];

    return <nav className="mt-4 border-t border-zinc-800 pt-3 overflow-x-auto no-scrollbar">
      <div className="grid grid-cols-5 gap-1 min-w-[560px] sm:min-w-0">
        {nav.map(([key, icon, label]) => <button
          key={key}
          onClick={() => setScreen(key)}
          className={`h-14 flex flex-col items-center justify-center gap-1 border-b-2 transition active:scale-[0.98] ${screen === key ? "border-yellow-400 text-yellow-400" : "border-transparent text-zinc-400"}`}
        >
  ...
  function PremiumNotificationsModal() {
    ...
    const visible = mapped
      .filter((n) => notificationTab === "Todas" || n.group === notificationTab)
      .slice(0, 20);
      ...
      <div className="w-full max-w-3xl bg-zinc-950 border border-yellow-500/40 rounded-t-[34px] md:rounded-[34px] shadow-2xl p-5 max-h-[88vh] overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between gap-3 mb-5">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-3xl font-black">Notificações</h2>
              <p className="text-zinc-400">Fique por dentro do que importa.</p>
            </div>
          </div>
        ...
        <div className="flex gap-2 overflow-x-auto pb-3 mb-3 no-scrollbar">
          {tabs.map((tab) => <button
            ...
            className={`px-5 py-3 rounded-2xl border whitespace-nowrap font-black transition ${notificationTab === tab ? "bg-yellow-500 text-black border-yellow-300" : "bg-zinc-900 border-zinc-700 text-zinc-300"}`}
          >
            {tab}
          </button>)}
        </div>
        ...
        <div className="overflow-y-auto pr-1 space-y-3">
          {visible.length === 0 ? <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-5 text-zinc-400">Nenhuma notificação nesta aba.</div> : visible.map((n, index) => <div key={n.id || index} className={`rounded-3xl p-4 border ${index === 0 ? "bg-yellow-500/10 border-yellow-400" : "bg-zinc-900 border-zinc-800"}`}>
            <div className="flex gap-4">
              <div className="w-14 h-14 rounded-full bg-black/40 border border-yellow-500/40 flex items-center justify-center text-2xl shrink-0">
                {n.group === "Caixa" ? "💰" : n.group === "Lista" ? "📋" : n.group === "Confirmações" ? "👤" : "🔔"}
              </div>
              <div className="min-w-0 flex-1">
                <div className="flex items-start justify-between gap-2">
                  <b className="text-lg leading-snug">{n.message}</b>
                  {index < 3 && <span className="bg-yellow-400 text-black rounded-lg px-2 py-1 text-[10px] font-black shrink-0">NOVO</span>}
                </div>
                <p className="text-sm text-zinc-500 mt-2">🕒 {n.created_at ? new Date(n.created_at).toLocaleString("pt-BR") : ""} • {n.group}</p>
              </div>
            </div>
          </div>)}
        </div>

        <button onClick={() => setShowNotifications(false)} className="w-full mt-4 rounded-2xl border border-yellow-500/40 bg-zinc-900 p-4 text-yellow-300 font-black">
          Fechar notificações
        </button>
      </div>
    </div>;
  ...
  function PremiumStat({ icon, label, value, sub = "" }) {
    return <div className="bg-zinc-900/80 border border-zinc-700 rounded-3xl p-4 min-h-[126px] shadow-xl">
      <div className="text-3xl text-yellow-400 mb-3">{icon}</div>
      <p className="text-zinc-400 text-sm">{label}</p>
      <b className="text-3xl font-black">{value}</b>
  ...
  function PremiumAdminDashboard() {
    ...
    return <div className="space-y-5">
      <div className={goldPanel + " p-5 md:p-7 overflow-hidden relative"}>
        <div className="relative z-10">
          <p className={miniLabel + " text-black/80"}>Painel de comando</p>
          <h2 className="text-4xl md:text-5xl font-black text-black mt-2">Amigos FC</h2>
          <p className="text-black/85 font-bold mt-2 text-lg">Lista, caixa, ranking e jogo em uma visão rápida.</p>
          <div className="grid grid-cols-2 gap-3 mt-6">
            <div className={glassCard + " p-4"}><p className="text-yellow-300 text-xs font-black uppercase">Status da lista</p><b className="text-2xl">{statusText}</b><p className="text-emerald-400 text-xs mt-1">● {listActive ? "Em andamento" : "Aguardando criação"}</p></div>
            <div className={glassCard + " p-4"}><p className="text-yellow-300 text-xs font-black uppercase">Na lista</p><b className="text-2xl">{totalMain} / 14</b><p className="text-zinc-300 text-xs mt-1">Jogadores</p></div>
            <div className={glassCard + " p-4"}><p className="text-yellow-300 text-xs font-black uppercase">Caixa</p><b className="text-2xl">{money(cashBalance)}</b><p className="text-zinc-300 text-xs mt-1">Saldo disponível</p></div>
            <div className={glassCard + " p-4"}><p className="text-yellow-300 text-xs font-black uppercase">Líder do mês</p><b className="text-2xl">{leader ? leader.nickname : "—"}</b><p className="text-zinc-300 text-xs mt-1">{leader ? `${leader.points} pontos` : "Nenhum líder ainda"}</p></div>
          </div>
          <button onClick={() => setScreen("stats")} className="w-full mt-5 rounded-2xl bg-black/60 border border-yellow-500/40 p-4 flex justify-between items-center text-white font-black">📈 Ver ranking completo <span className="text-yellow-400 text-2xl">›</span></button>
        </div>
      </div>
    </div>;
  }

  function PremiumAthleteDashboard() {
    const pos = currentPlayer ? getPlayerListPosition(currentPlayer.id) : { type: "none" };
    const statusLabel = currentPlayer?.status === "jogo" ? "Confirmado" : currentPlayer?.status === "jogo_resenha" ? "Jogo + Resenha" : currentPlayer?.status === "resenha" ? "Só Resenha" : currentPlayer?.status === "nao_vou" ? "Não vou" : "Pendente";
    const positionText = pos.type === "main" ? `Na lista #${pos.index}` : pos.type === "reserve" ? `Suplente #${pos.index}` : "Fora da lista";
    const rankLabel = currentPlayer?.position === "Goleiro" ? "Jogos no mês" : "Ranking do mês";

    return <div className="space-y-5">
      <div className={goldPanel + " p-5 overflow-hidden relative"}>
        <div className="relative z-10">
          <p className={miniLabel + " text-black/80"}>Meu painel</p>
          <h2 className="text-4xl font-black text-black mt-2">{loggedPlayerName}</h2>
          <div className="grid grid-cols-2 gap-3 mt-6">
            <div className={glassCard + " p-4"}><p className="text-yellow-300 text-sm font-bold">Status</p><b className="text-xl">{statusLabel}</b></div>
            <div className={glassCard + " p-4"}><p className="text-yellow-300 text-sm font-bold">Lista</p><b className="text-xl">{positionText}</b></div>
            <div className={glassCard + " p-4"}><p className="text-yellow-300 text-sm font-bold">Pagamento</p><b className="text-xl">{paymentLabelFor(currentPlayer)}</b>{currentPlayer?.position === "Goleiro" && <span className="inline-block mt-2 px-3 py-1 rounded-full bg-yellow-400/20 text-yellow-300 text-xs font-black">GOLEIRO</span>}</div>
            <div className={glassCard + " p-4"}><p className="text-yellow-300 text-sm font-bold">{rankLabel}</p><b className="text-xl">{athleteRankingInfo()}</b></div>
          </div>
        </div>
      </div>
    </div>;
  ...
  function PremiumAthleteListBox() {
    if (!currentPlayer?.id) return null;
    const confirmed = mainGamePlayers.length;
    const pending = players.filter((p) => p.status === "pendente").length;
    const absences = players.filter((p) => p.status === "nao_vou").length;

    return <div className={premiumCard + " p-5 space-y-4"}>
      {!listActive && <div className="bg-zinc-800 border border-zinc-700 rounded-2xl p-4 text-zinc-300 font-bold">⏳ Não existe nenhuma lista aberta, aguarde a liberação para alterar seu status.</div>}
      {listActive && <div>
        <div className="flex gap-4 items-start">
          <div className="w-16 h-16 rounded-2xl bg-yellow-500/15 border border-yellow-500 flex items-center justify-center text-3xl shrink-0">📋</div>
          <div className="flex-1"><h2 className="text-3xl font-black">Lista ativa</h2><p className="text-zinc-400 text-sm">📅 {gameDate ? new Date(gameDate + "T12:00:00").toLocaleDateString("pt-BR") : "-"} • 🕒 {gameTime} • 📍 {gameLocation || "Local não informado"}</p><div className="flex flex-wrap gap-3 mt-3"><span className={`${listCanReceiveStatus ? "bg-emerald-500" : "bg-red-500"} text-black px-4 py-1.5 rounded-full font-black text-sm`}>{listCanReceiveStatus ? "ABERTA" : "BLOQUEADA"}</span><span className="text-zinc-300 text-sm">⏱️ Fecha em <b className="text-yellow-400">{formatCountdown()}</b></span></div></div>
        </div>
        <div className="grid grid-cols-4 gap-2 text-center border-y border-zinc-700 py-4 mt-4">
          <div><b className="text-emerald-400 text-xl">{confirmed}</b><p className="text-xs text-zinc-400">Confirmados</p></div>
          <div><b className="text-yellow-400 text-xl">{pending}</b><p className="text-xs text-zinc-400">Pendentes</p></div>
          <div><b className="text-red-400 text-xl">{absences}</b><p className="text-xs text-zinc-400">Ausências</p></div>
          <div><b className="text-white text-xl">{players.length}</b><p className="text-xs text-zinc-400">Total</p></div>
        </div>
      </div>}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <button disabled={!listCanReceiveStatus} onClick={() => updatePlayerStatus(currentPlayer.id, "jogo")} className="rounded-2xl border border-yellow-500 bg-yellow-500/10 p-4 font-black text-left disabled:opacity-40">⚽ Vou no jogo</button>
        <button disabled={!listCanReceiveStatus} onClick={() => updatePlayerStatus(currentPlayer.id, "jogo_resenha")} className="rounded-2xl border border-yellow-500 bg-zinc-900 p-4 font-black text-left disabled:opacity-40">🥩⚽ Jogo + Resenha</button>
        <button disabled={!listCanReceiveStatus} onClick={() => updatePlayerStatus(currentPlayer.id, "resenha")} className="rounded-2xl border border-yellow-500 bg-zinc-900 p-4 font-black text-left disabled:opacity-40">🥩 Só Resenha</button>
        <button disabled={!listCanReceiveStatus} onClick={() => updatePlayerStatus(currentPlayer.id, "nao_vou")} className="rounded-2xl border border-zinc-600 bg-zinc-900 p-4 font-black text-left disabled:opacity-40">✕ Não vou</button>
      </div>
    </div>;
  }

  function PremiumAthleteStats() {
    const stats = currentPlayerMonthlyStats();
    const games = stats?.games || 0;
    return <div className={premiumCard + " p-5"}>
      <h2 className="text-2xl font-black mb-4">📊 Estatísticas rápidas</h2>
      <div className="grid grid-cols-2 gap-3">
        <PremiumStat icon="📅" label="Jogos no mês" value={games} />
        <PremiumStat icon="⚽" label="Gols" value={stats?.goals || 0} />
        <PremiumStat icon="👟" label="Assistências" value={stats?.assists || 0} />
        <PremiumStat icon="👤" label="Presenças" value={games ? "100%" : "0%"} sub={games ? `${games} jogos` : ""} />
      </div>
    </div>;
  }

  async function promoteAdmin(playerId) {
    if (!isAdmin) return;
    const updated = players.map((p) => String(p.id) === String(playerId) ? { ...p, role: "admin", approved: true } : p);
    const { error } = await supabase.from("players").upsert(updated);
    if (error) return notifyListChange("Erro ao tornar admin: " + error.message);
    setPlayers(updated);
    createAuditLog("admin_promoted", { playerId });
    notifyListChange("Usuário promovido a Admin.");
  }

  async function demoteAdmin(playerId) {
    if (!isAdmin) return;
    const target = players.find((p) => String(p.id) === String(playerId));
    if (String(target?.email || "").toLowerCase() === SUPER_ADMIN_EMAIL && !isSuperAdmin) return notifyListChange("Super Admin não pode ser alterado.");
    const updated = players.map((p) => String(p.id) === String(playerId) ? { ...p, role: "athlete" } : p);
    const { error } = await supabase.from("players").upsert(updated);
    if (error) return notifyListChange("Erro ao tirar admin: " + error.message);
    setPlayers(updated);
    createAuditLog("admin_demoted", { playerId });
    notifyListChange("Permissão de Admin removida.");
  }

  function SecurityScreen() {
    if (!isAdmin) {
      return <div className={premiumCard + " p-5"}><div className="bg-red-950/50 border border-red-700 rounded-2xl p-4 text-red-300 font-black">Acesso permitido somente para Admin.</div></div>;
    }

    const q = securitySearch.toLowerCase();
    const filteredPlayers = players.filter((p) => `${p.name} ${p.nickname || ""} ${p.email || ""}`.toLowerCase().includes(q));
    ...
    return <div className="space-y-5">
      <div className={premiumCard + " p-5"}>
        <div className="flex items-center gap-4 mb-5">
          ...
          <div>
            <h2 className="text-3xl font-black">Segurança e permissões</h2>
            <p className="text-zinc-400">Gerencie usuários, acessos e permissões do sistema.</p>
          </div>
        </div>

        <div className="grid md:grid-cols-3 gap-3 mb-5">
          <PremiumStat icon="🛡️" label="Admins" value={admins.length} />
          <PremiumStat icon="💰" label="Financeiro" value={finance.length} />
          <PremiumStat icon="⏳" label="Pendentes" value={pending.length} />
        </div>

        <div className="flex gap-2 mb-5">
          <input className={inputCls + " flex-1"} placeholder="Buscar usuário..." value={securitySearch} onChange={(e) => setSecuritySearch(e.target.value)} />
          <button onClick={refreshSecurityData} className={iconBtnGold}>⚙️</button>
        </div>

        <div className="space-y-3">
          {filteredPlayers.map((p) => {
            const isProtected = String(p.email || "").toLowerCase() === SUPER_ADMIN_EMAIL && SUPER_ADMIN_EMAIL;
            return <div key={p.id} className="bg-zinc-900 border border-zinc-800 rounded-3xl p-4">
              <div className="flex items-start gap-4">
                <div className="w-16 h-16 rounded-full bg-black border border-yellow-500 flex items-center justify-center text-yellow-400 text-2xl font-black shrink-0">
                  {initials(displayName(p))}
                </div>

                <div className="min-w-0 flex-1">
                  <h3 className="text-2xl font-black truncate">{displayName(p)}</h3>
                  <p className="text-zinc-400 truncate">{p.email || "-"}</p>
                  <div className="flex flex-wrap gap-2 mt-2">
                    {isProtected && <span className="px-3 py-1 rounded-xl bg-yellow-500 text-black text-xs font-black">👑 Super Admin</span>}
                    {p.role === "admin" && <span className="px-3 py-1 rounded-xl bg-emerald-500/20 text-emerald-300 border border-emerald-600 text-xs font-black">Admin</span>}
                    {p.finance_responsible && <span className="px-3 py-1 rounded-xl bg-yellow-500/20 text-yellow-300 border border-yellow-600 text-xs font-black">Financeiro</span>}
                    {p.approved !== false && <span className="px-3 py-1 rounded-xl bg-blue-500/20 text-blue-300 border border-blue-600 text-xs font-black">Aprovado</span>}
                  </div>
                </div>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 mt-4">
                <button disabled={isProtected && !isSuperAdmin} onClick={() => p.role === "admin" ? demoteAdmin(p.id) : promoteAdmin(p.id)} className="rounded-2xl bg-zinc-800 border border-zinc-700 p-3 font-bold disabled:opacity-40">
                  🛡️ {p.role === "admin" ? "Tirar admin" : "Tornar admin"}
                </button>
                <button onClick={() => p.finance_responsible ? removeFinanceResponsible() : setFinanceResponsible(p.id)} className="rounded-2xl bg-zinc-800 border border-yellow-600/50 p-3 font-bold">
                  💰 {p.finance_responsible ? "Tirar financeiro" : "Financeiro"}
                </button>
                <button onClick={() => p.approved === false ? approvePlayer(p.id) : notifyListChange("Usuário já aprovado.")} className="rounded-2xl bg-zinc-800 border border-emerald-600/50 p-3 font-bold">
                  ✅ {p.approved === false ? "Aprovar" : "Aprovado"}
                </button>
              </div>
            </div>;
          })}
      ...
      <div className={premiumCard + " p-5"}>
        <h2 className="text-2xl font-black mb-4">📜 Logs de auditoria</h2>
        {auditLogs.length === 0 ? <p className="text-zinc-500">Nenhum log registrado ainda.</p> : <div className="space-y-2 max-h-[420px] overflow-auto pr-1">
          {auditLogs.map((log) => <div key={log.id} className="bg-zinc-900 border border-zinc-800 rounded-2xl p-3">
            <div className="flex justify-between gap-3">
              <b>{log.action}</b>
              <span className="text-xs text-zinc-500">{log.created_at ? new Date(log.created_at).toLocaleString("pt-BR") : ""}</span>
            </div>
            <p className="text-xs text-zinc-400 mt-1">{log.actor_email || "-"}</p>
          </div>)}
        </div>}
      </div>
    </div>;
