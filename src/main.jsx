...
class AppErrorBoundary extends React.Component {
  ...
  static getDerivedStateFromError(error) {
    return { hasError: true, errorMessage: error?.message || "Erro inesperado no app." };
  }

  componentDidCatch(error, info) {
    console.error("App dos Amigos FC - erro capturado:", error, info);
  }

  render() {
    if (this.state.hasError) {
      return <main className="min-h-screen bg-black text-white p-4 flex items-center justify-center">
        <div className="bg-zinc-900 border border-red-700 rounded-3xl p-6 max-w-md w-full space-y-4 shadow-2xl">
          <div className="text-5xl">⚠️</div>
          <h1 className="text-2xl font-black">O app encontrou um erro</h1>
          <p className="text-zinc-300">Não ficou tela preta. Recarregue o app e tente novamente.</p>
          <pre className="bg-black/50 border border-zinc-800 rounded-2xl p-3 text-xs text-red-300 overflow-auto">{this.state.errorMessage}</pre>
          <button onClick={() => window.location.reload()} className="w-full rounded-2xl bg-yellow-500 text-black font-black p-4">Recarregar app</button>
        </div>
      </main>;
    }
...
function AppDosAmigosFC() {
  ...
  function getVisibleCashBalance() {
    const movements = Array.isArray(cashMovements) ? cashMovements : [];
  ...
  async function loadCashMovements() {
    ...
    const loadedBalance = (data || []).reduce((sum, m) => sum + Number(m.amount || 0), 0);
    setCashBalance(loadedBalance);
  }
  ...
  async function deleteCashMovement(id) {
    ...
    const amountToRemove = Number(movement?.amount || 0);

    const { error } = await supabase.from("cash_movements").delete().eq("id", id);
    if (error) return notifyListChange("Erro ao remover lançamento: " + error.message);

    const remainingMovements = cashMovements.filter((m) => String(m.id) !== String(id));
    const nextBalance = remainingMovements.reduce((sum, m) => sum + Number(m.amount || 0), 0);

    setCashMovements(remainingMovements);
    setCashBalance(nextBalance);
    await saveSettings({ cashBalance: nextBalance });
    await loadCashMovements();

    if (typeof createAuditLog === "function") {
      createAuditLog("cash_movement_deleted", { id, amount: amountToRemove, balance_after: nextBalance });
    }
        ...
        <div className="bg-gradient-to-br from-yellow-500 via-yellow-600 to-zinc-950 rounded-3xl p-5 md:p-7 shadow-2xl border border-yellow-400/40 overflow-hidden relative">
          ...
          <div className="relative z-10">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mt-5">
              ...
              <div className="bg-black/25 backdrop-blur rounded-2xl p-3 text-white"><p className="text-xs opacity-70">Caixa</p><b>{money(getVisibleCashBalance())}</b></div>
  ...
  function PlayerCardPremium({ p, index }) {
    ...
    return <div className="bg-gradient-to-br from-zinc-900 to-black border border-zinc-800 rounded-[28px] p-4 shadow-xl">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <p className="text-[11px] text-zinc-500 font-black">#{index + 1}</p>
              <h3 className="text-lg font-black truncate leading-tight">{p.name}</h3>
            ...
            <button onClick={() => { setEditId(p.id); setForm({ name: p.name, nickname: p.nickname || "", email: p.email || "", tempPassword: "", level: p.level, position: p.position }); }} className="rounded-xl bg-blue-500/15 border border-blue-500/30 text-blue-300 px-3 py-2 text-xs font-black shrink-0">
              Editar
            </button>
          </div>
          ...
          <div className="flex flex-wrap gap-2 mt-3">
            ...
            {p.approved === false ? <PremiumPill tone="red">Pendente</PremiumPill> : <PremiumPill tone="green">Aprovado</PremiumPill>}
          </div>
        </div>
      </div>
      ...
      <div className="mt-4 grid grid-cols-1 gap-2">
        <div className="grid grid-cols-2 gap-2">
          <button disabled={protectedSuper} onClick={() => setAdminRole(p.id)} className={`rounded-2xl border px-3 py-3 text-sm font-black active:scale-[0.97] transition disabled:opacity-40 ${p.role === "admin" ? "bg-yellow-500/10 border-yellow-500/40 text-yellow-300" : "bg-zinc-900 border-zinc-700 text-zinc-200"}`}>
            🛡️ {p.role === "admin" ? "Tirar admin" : "Tornar admin"}
          </button>

          <button onClick={() => p.finance_responsible ? removeFinanceResponsible() : setFinanceResponsible(p.id)} className={`rounded-2xl border px-3 py-3 text-sm font-black active:scale-[0.97] transition ${p.finance_responsible ? "bg-emerald-500/10 border-emerald-500/40 text-emerald-300" : "bg-zinc-900 border-zinc-700 text-zinc-200"}`}>
            💰 {p.finance_responsible ? "Tirar financeiro" : "Financeiro"}
          </button>
        </div>

        <div className="grid grid-cols-2 gap-2">
          <button disabled={p.approved !== false} onClick={() => approvePlayer(p.id)} className={`rounded-2xl border px-3 py-3 text-sm font-black active:scale-[0.97] transition disabled:opacity-50 ${p.approved === false ? "bg-emerald-500 text-black border-emerald-300" : "bg-emerald-500/10 border-emerald-500/40 text-emerald-300"}`}>
            ✅ {p.approved === false ? "Aprovar" : "Aprovado"}
          </button>

          <button disabled={protectedSuper} onClick={() => askRemovePlayer(p)} className="rounded-2xl bg-red-500/10 border border-red-500/35 text-red-300 px-3 py-3 text-sm font-black active:scale-[0.97] transition disabled:opacity-40">
            🗑️ Remover
          </button>
        </div>
      </div>
  ...
  function PremiumHeaderStable() {
    const unreadCount = (smartNotifications?.length || 0) || (liveEvents?.length || 0);
  ...
  if (!user) return <main className="min-h-screen bg-zinc-950 text-white flex items-center justify-center p-4"><div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-6 w-full max-w-md space-y-4 shadow-2xl"><div className="text-center space-y-2"><img src={APP_ICON_SRC} alt="Amigos FC" className="w-24 h-24 mx-auto rounded-3xl shadow-2xl border border-yellow-400/40 object-cover" /><h1 className="text-3xl font-black">App dos Amigos FC</h1><p className="text-zinc-400 text-xs md:text-base">Entre ou crie seu acesso.</p></div>{authView === "signup" && <><Field label="Nome completo"><input className={inputCls} value={authForm.name} onChange={(e) => setAuthForm({ ...authForm, name: e.target.value.toUpperCase() })} /></Field><Field label="Nome/Apelido (lista)"><input className={inputCls} value={authForm.nickname} onChange={(e) => setAuthForm({ ...authForm, nickname: e.target.value.toUpperCase() })} /></Field></>}<Field label="E-mail"><input className={inputCls} type="email" value={authForm.email} onChange={(e) => setAuthForm({ ...authForm, email: e.target.value })} /></Field><Field label="Senha"><input className={inputCls} type="password" value={authForm.password} onChange={(e) => setAuthForm({ ...authForm, password: e.target.value })} /></Field>{authView === "signup" && <Field label="Posição"><select className={inputCls} value={authForm.position} onChange={(e) => setAuthForm({ ...authForm, position: e.target.value })}><option value="Linha">Linha</option><option value="Goleiro">Gol</option></select></Field>}
              <label className="flex items-center justify-between gap-3 bg-zinc-900 border border-zinc-700 rounded-2xl p-3 text-left">
                <span>
                  <b>Lembrar e-mail neste aparelho</b>
                  <p className="text-zinc-500 text-xs">Facilita os próximos acessos sem salvar senha.</p>
                </span>
                <input type="checkbox" checked={rememberLogin} onChange={(e) => updateRememberLogin(e.target.checked)} className="w-5 h-5 accent-yellow-500" />
              </label>

              {authMsg && <p className="text-yellow-400 text-sm font-bold">{authMsg}</p>}<Button onClick={authView === "login" ? signIn : signUp} className="bg-emerald-500 text-black w-full">{authView === "login" ? "Entrar" : "Criar acesso"}</Button><button className="text-sm text-emerald-400 font-bold w-full" onClick={() => setAuthView(authView === "login" ? "signup" : "login")}>{authView === "login" ? "Criar novo acesso" : "Já tenho acesso"}</button></div></main>;
...
<p className="text-xs text-zinc-400 -mt-2">Para criar login, informe e-mail e senha temporária. No primeiro acesso o atleta será obrigado a trocar a senha.</p><div className="grid md:grid-cols-[1fr_auto] gap-3 items-end"><Field label="Buscar jogador"><input className={inputCls} value={playerSearch} onChange={(e) => setPlayerSearch(e.target.value)} /></Field><Field label="Filtro"><select className={inputCls} value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}><option value="todos">Todos</option>{statusList.map((s) => <option key={s[0]} value={s[0]}>{s[1]}</option>)}</select></Field></div><div className="space-y-3">
  ...
  {screen === "cash" && canAccessCash && <Box title="💰 Caixa">
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
      <StatCard label="Saldo atual" value={money(getVisibleCashBalance())} tone={getVisibleCashBalance() >= 0 ? "bg-emerald-950/60" : "bg-red-950/60"} />
...
createRoot(document.getElementById("root")).render(<React.StrictMode><AppErrorBoundary><AppDosAmigosFC /></AppErrorBoundary></React.StrictMode>);
