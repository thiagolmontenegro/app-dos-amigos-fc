...
function AppDosAmigosFC() {
    ...
    return <header className="bg-zinc-950/95 border border-zinc-800 rounded-[24px] p-3 shadow-2xl sticky top-2 z-50 backdrop-blur">
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-zinc-500 text-xs whitespace-nowrap">{effectiveMode === "admin" ? "Modo Admin" : "Perfil Atleta"}</p>
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          {isAdmin && <button className="w-10 h-10 rounded-full border border-zinc-700 bg-zinc-950/80 flex items-center justify-center text-base shadow-lg active:scale-95 transition relative" onClick={() => setMode(mode === "admin" ? "atleta" : "admin")} title="Modo">
            🧑‍💼
          </button>}

          {isAdmin && <button className="w-10 h-10 rounded-full border border-zinc-700 bg-zinc-950/80 flex items-center justify-center text-base shadow-lg active:scale-95 transition" onClick={() => setHide(!hide)} title="Níveis">
            👁️
          </button>}
  ...
  function PremiumNavStable() {
    ...
    return <nav className="mt-3 border-t border-zinc-800 pt-2">
      <div className={isAdmin ? "grid grid-cols-5 gap-0.5" : "grid grid-cols-2 gap-2"}>
        {nav.map(([key, icon, label]) => <button
          key={key}
          onClick={() => setScreen(key)}
          className={`h-[58px] rounded-xl flex flex-col items-center justify-center gap-0.5 border transition active:scale-[0.97] ${screen === key ? "bg-yellow-500/15 border-yellow-400 text-yellow-400 shadow-lg shadow-yellow-500/10" : "bg-zinc-900 border-zinc-800 text-zinc-400"}`}
        >
