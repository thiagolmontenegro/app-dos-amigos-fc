...
function AppDosAmigosFC() {
  ...
  function PremiumHeader() {
    ...
    return <header className="bg-zinc-950/90 border border-zinc-800 rounded-[28px] p-4 shadow-2xl sticky top-2 z-50 backdrop-blur">
      <div className="flex items-center gap-3">
        <img src={APP_ICON_SRC} alt="Amigos FC" className="w-14 h-14 sm:w-16 sm:h-16 rounded-2xl object-cover border border-yellow-400/50 shadow-lg shrink-0" />
        <div className="min-w-0 flex-1">
          <h1 className="text-2xl sm:text-3xl font-black leading-tight">
            App dos <span className="text-yellow-400">Amigos FC</span>
          </h1>
          <p className="text-emerald-400 font-bold text-sm mt-1">● Banco conectado</p>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          <button className={iconBtnGold + " relative"} onClick={() => setShowNotifications(true)} title="Notificações">
            🔔
            {unreadCount > 0 && <span className="absolute -top-1 -right-1 min-w-5 h-5 px-1 rounded-full bg-yellow-300 text-black text-xs font-black flex items-center justify-center">{Math.min(unreadCount, 9)}</span>}
          </button>
          {isAdmin && <button className={iconBtn} onClick={() => setMode(mode === "admin" ? "atleta" : "admin")} title="Modo">{effectiveMode === "admin" ? "🧑‍💼" : "👤"}</button>}
          {isAdmin && <button className={iconBtn} onClick={() => setHide(!hide)} title="Níveis">👁️</button>}
          {user && <button className={iconBtn} onClick={signOut} title="Sair">↪</button>}
        </div>
      </div>
  ...
  function PremiumNav() {
    const nav = isAdmin
      ? [["home", "🏠", "Início", "Visão geral"], ["players", "👥", "Jogadores", "Gerencie"], ["newGame", "🆕", "Novo jogo", "Criar"], ["match", "⚽", "Times", "Partida"], ["stats", "📊", "Estatísticas", "Ranking"], ["cash", "💰", "Caixa", "Finanças"], ["security", "🛡️", "Segurança", "Acessos"]]
      : [["home", "🏠", "Início", "Meu painel"], ["cash", "💰", "Caixa", "Finanças"]];

    return <nav className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-2 mt-5">
      {nav.map(([key, icon, label, sub]) => <button key={key} onClick={() => setScreen(key)} className={`rounded-2xl p-3 min-h-[76px] border text-left transition active:scale-[0.98] ${screen === key ? "bg-yellow-500/20 border-yellow-400 shadow-lg shadow-yellow-500/10" : "bg-zinc-900 border-zinc-700"}`}>
        <div className="flex items-center gap-3">
          <span className="text-2xl">{icon}</span>
          <div><b className={screen === key ? "text-yellow-300" : "text-white"}>{label}</b><p className="text-xs text-zinc-400">{sub}</p></div>
        </div>
      </button>)}
    </nav>;
  }
  ...
  function PremiumNotificationsModal() {
    ...
    const visible = smartNotifications.filter((n) => n.audience === "finance" ? canAccessCash : true).slice(0, 12);

    return <div className="fixed inset-0 z-[100] bg-black/70 backdrop-blur-sm flex items-end md:items-center justify-center p-3">
      <div className="w-full max-w-3xl bg-zinc-950 border border-yellow-500/40 rounded-t-[34px] md:rounded-[34px] shadow-2xl p-5 max-h-[88vh] overflow-auto">
        <div className="w-16 h-1.5 rounded-full bg-zinc-700 mx-auto mb-5 md:hidden"></div>
        <div className="flex items-center justify-between gap-3 mb-5">
          <div className="flex items-center gap-4">
            <div className="w-16 h-16 rounded-full bg-yellow-500/15 border border-yellow-400 flex items-center justify-center text-3xl">🔔</div>
            <div><h2 className="text-3xl font-black">Notificações</h2><p className="text-zinc-400">Fique por dentro do que importa.</p></div>
          </div>
          <button onClick={() => setShowNotifications(false)} className={iconBtn}>✕</button>
        </div>
        <div className="flex gap-2 overflow-x-auto pb-2 mb-4">
          {["Todas", "Lista", "Confirmações", "Caixa", "Sistema"].map((x, i) => <span key={x} className={`px-5 py-3 rounded-2xl border whitespace-nowrap font-bold ${i === 0 ? "bg-yellow-500 text-black border-yellow-300" : "bg-zinc-900 border-zinc-700 text-zinc-300"}`}>{x}</span>)}
        </div>
        {visible.length === 0 ? <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-5 text-zinc-400">Nenhuma notificação registrada.</div> : <div className="space-y-3">
          {visible.map((n, index) => <div key={n.id || index} className={`rounded-3xl p-4 border ${index === 0 ? "bg-yellow-500/10 border-yellow-400" : "bg-zinc-900 border-zinc-800"}`}>
            <div className="flex gap-4">
              <div className="w-14 h-14 rounded-full bg-black/40 border border-yellow-500/40 flex items-center justify-center text-2xl shrink-0">{n.type?.includes("cash") ? "💰" : n.type?.includes("list") ? "📋" : n.type?.includes("status") ? "👤" : "🔔"}</div>
              <div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-2"><b className="text-lg leading-snug">{n.message}</b>{index < 3 && <span className="bg-yellow-400 text-black rounded-lg px-2 py-1 text-[10px] font-black">NOVO</span>}</div><p className="text-sm text-zinc-500 mt-2">🕒 {n.created_at ? new Date(n.created_at).toLocaleString("pt-BR") : ""}</p></div>
            </div>
          </div>)}
        </div>}
        <button onClick={() => setShowNotifications(false)} className="w-full mt-5 rounded-2xl border border-yellow-500/40 bg-zinc-900 p-4 text-yellow-300 font-black">Fechar notificações</button>
      </div>
    </div>;
  }
  ...
  return <main className="min-h-screen bg-black text-white p-3 sm:p-4 md:p-8 space-y-5 md:space-y-6 pb-24"><PremiumHeader />
