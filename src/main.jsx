...
function AppDosAmigosFC() {
  ...
  async function saveCurrentGameHistory() {
    ...
    const payload = buildCurrentGamePayload();
  ...
  function StatsScreen() {
    ...
    return <div className="space-y-6">
      ...
      <Box title="🗂️ Histórico de jogos salvos">
        {savedMatches.length === 0 ? <p className="text-zinc-400">Nenhum jogo salvo ainda.</p> : <div className="space-y-2">
          {savedMatches.map((match) => <div key={match.id} className="bg-zinc-800 border border-zinc-700 rounded-2xl p-3 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-zinc-400">{match.location || "Local não informado"} • MVP: {match.mvp || "-"}</p>
            </div>
  ...
  function Header() {
    const nav = [["home", "🏠 Início"], ["players", "👥 Jogadores"], ["newGame", "🆕 Novo jogo"], ["match", "⚽ Times"], ["stats", "📊 Estatísticas"], ["cash", "💰 Caixa"]];
    ...
    return <header className="bg-gradient-to-br from-zinc-900 to-zinc-950 border border-zinc-800 rounded-3xl p-5 shadow-2xl space-y-4 sticky top-0 z-40 backdrop-blur">
      ...
      {effectiveMode === "admin" && <nav className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2">{nav.map(([key, label]) => <button key={key} onClick={() => setScreen(key)} className={`rounded-2xl px-3 py-3 font-black text-sm transition border ${screen === key ? "bg-emerald-500 text-black border-emerald-300" : "bg-zinc-800 text-white border-zinc-700 hover:bg-zinc-700"}`}>{label}</button>)}</nav>}
    </header>;
  }
